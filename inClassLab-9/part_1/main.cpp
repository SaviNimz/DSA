...
struct HashTable{
    int MAX_LENGTH = 4;
    // this will allocate memory from the heap
    ...
    int hashfunc(string user_name){
        ...
        // we will get the sum of the ascii values of each character in the user name string
    ...
    // check if the position related to the hash value is empty and if its empty assign the value of password in that index
        if (password[hash].empty()){
